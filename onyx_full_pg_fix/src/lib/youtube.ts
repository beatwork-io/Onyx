...
export async function uploadAndSchedule({channelId,filePath,title,description,tags,playlistId,publishAt}:{channelId:string;filePath:string;title:string;description:string;tags:string[];playlistId?:string;publishAt:Date;}) {
  const { yt } = await youtubeClientForChannel(channelId);
  const res = await yt.videos.insert({
    part: ["snippet","status"],
    requestBody: { snippet: { title, description, tags, categoryId: "10"}, status: { privacyStatus: "private", publishAt: publishAt.toISOString(), selfDeclaredMadeForKids: false } },
...
export async function patchDescription(channelId: string, videoId: string, description: string) {
  const { yt } = await youtubeClientForChannel(channelId);
  await yt.videos.update({ part: ["snippet"], requestBody: { id: videoId, snippet: { description, title: undefined } } });
}
