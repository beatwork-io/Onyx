...
def estimate_bpm(y, sr):
    ...
    if bpm < 70: bpm *= 2
    if bpm > 180: bpm /= 2
    conf = min(1.0, len(beats)/ (len(y)/sr/0.5 + 1e-6))
    return bpm, conf
...
strategy = 'tonal'; key = key_tonal; conf = ckey
if ckey < 0.25:
    key = key_808; conf = c808; strategy = '808'
print(json.dumps({"bpm": float(bpm),"key": key,"confidence_bpm": float(cbpm),"confidence_key": float(conf),"strategy": strategy}))
