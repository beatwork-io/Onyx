...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  // Create DB row
  let beat = await prisma.beat.create({ data: {
    clientId, channelId, title: "", description: "", tags: [], fileAudio: audioPath, fileCover: coverPath, status: "ANALYZING"
  }});
  ...
  try {
    ...
    beat = await prisma.beat.update({ where: { id: beat.id }, data: {
      bpm: bpm ?? undefined,
      key: Key ?? undefined,
      confidenceBpm: an.confidence_bpm,
      confidenceKey: an.confidence_key,
      keyStrategy: an.strategy,
      title, description,
      status: "RENDERING"
    }});
    ...
    if (!publishAt) publishAt = new Date(Date.now() + 48 * 3600 * 1000); // fail-safe 48h later
