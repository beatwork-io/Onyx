...
export default function BeatForm(){
  ...
  async function onSubmit(e:any){ e.preventDefault(); setBusy(true); const fd=new FormData(e.currentTarget); const r=await fetch("/api/beat/create",{method:"POST", body:fd}); const j=await r.json(); setResult(j); setBusy(false); }
  return (<form onSubmit={onSubmit} className="max-w-2xl mx-auto space-y-4 p-6 border rounded-2xl shadow">
