...
export function renderVideoToStream({
  ...
  const fontPath = "/app/public/fonts/SF-Pro-Display-Bold.ttf"; // mets ton .ttf ici dans le repo
  ...
  const drawText = hasFont
    ? `drawtext=fontfile=${fontPath}:text:'${titleOverlay
        .replace(/:/g, "\\:")
        .replace(/'/g, "\\'")}':x=(w-text_w)/2:y=80:fontsize=48:fontcolor=white:shadowcolor=black:shadowx=2:shadowy=2`
    : null; // pas de police => pas de texte => pas d'erreur fontconfig
  ...
  const bin = (ffmpegPath as string) || "ffmpeg"; // ffmpeg-static → pas besoin d’apt-get
